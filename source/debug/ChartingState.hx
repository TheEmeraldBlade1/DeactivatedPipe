	...
	**/
	...
	override function update(elapsed:Float)
	{
		...
		if (!anyTextHasFocus)
		{
			...
			if (FlxG.mouse.overlaps(curRenderedNotes) && !holding && !FlxG.mouse.pressed 
				|| FlxG.mouse.overlaps(curRenderedEvents) && !holding && !FlxG.mouse.pressed){
						Cursor.createPointer();
					}
					else if (FlxG.mouse.overlaps(gridBG) && !holding && !FlxG.mouse.pressed){
						Cursor.createCell();
					}else{
