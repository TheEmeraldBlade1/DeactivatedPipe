...
class FunkinLuaOLD {
	...
	public function new(script:String) {
		...
		Lua_helper.add_callback(lua, "addScore", function(value:Int = 0) {
			lePlayState.songStats.score += value;
			lePlayState.updateAccuracy();
		});
		Lua_helper.add_callback(lua, "addMisses", function(value:Int = 0) {
			lePlayState.songStats.missCount += value;
			lePlayState.updateAccuracy();
		});
		...
		Lua_helper.add_callback(lua, "getCharacterX", function(type:String) {
			switch(type.toLowerCase()) {
				case 'dad' | 'opponent':
					return lePlayState.dad.x;
				case 'gf' | 'girlfriend':
					return lePlayState.gf.x;
				default:
					return lePlayState.boyfriend.x;
			}
		});
		Lua_helper.add_callback(lua, "setCharacterX", function(type:String, value:Float) {
			switch(type.toLowerCase()) {
				case 'dad' | 'opponent':
					lePlayState.dad.x = value;
				case 'gf' | 'girlfriend':
					lePlayState.gf.x = value;
				default:
					lePlayState.boyfriend.x = value;
			}
		});
		Lua_helper.add_callback(lua, "getCharacterY", function(type:String) {
			switch(type.toLowerCase()) {
				case 'dad' | 'opponent':
					return lePlayState.dad.y;
				case 'gf' | 'girlfriend':
					return lePlayState.gf.y;
				default:
					return lePlayState.boyfriend.y;
			}
		});
		Lua_helper.add_callback(lua, "setCharacterY", function(type:String, value:Float) {
			switch(type.toLowerCase()) {
				case 'dad' | 'opponent':
					lePlayState.dad.y = value;
				case 'gf' | 'girlfriend':
					lePlayState.gf.y = value;
				default:
					lePlayState.boyfriend.y = value;
			}
		});
		...
		Lua_helper.add_callback(lua, "setRatingPercent", function(value:Float) {
			lePlayState.songStats.accuracy = value;
		});
			...
			if(lePlayState.modchartSprites.exists(tag)) {
				...
				if(!shit.wasAdded) {
					if(front) {
						...
						var position:Int = lePlayState.members.indexOf(lePlayState.gf);
						if(lePlayState.members.indexOf(lePlayState.boyfriend) < position) {
							position = lePlayState.members.indexOf(lePlayState.boyfriend);
						} else if(lePlayState.members.indexOf(lePlayState.dad) < position) {
							position = lePlayState.members.indexOf(lePlayState.dad);
						}
